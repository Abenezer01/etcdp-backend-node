...
self.actionUserFinder = async (model,action, user_id, department_id) => {
  try {
    ...
    let posIds = await Promise.all(pos.map(async (item) => {
...
self.notifyActor = async (act, action, user_id, department_id) => {
    ...
    let userIds = await self.actionUserFinder(act.model, `${action}`, user_id, department_id)

    if (userIds.length) {
        let notifications = await Promise.all(userIds.map(async (usr) => {
            let ac = action.toUpperCase()
            return await notificationHelper.notify(`${ac}`, `Please ${action} data added on ${act.model}`, act.model, usr, modelvalue, `You are required to ${action} this data`)
        }))

...
self.getAll = async(req, res) => {
    ...
    try {
        const { rows, count } = await projecttype.findAndCountAll({
            limit,
            offset,
            order: [
                ['createdAt', order]
...
self.get = async(req, res) => {
    try {
        ...
        let data = await projecttype.findOne({
            where: {
                id: id
            }
        });
...
self.save = async(req, res) => {
    try {
        ...
        if (usr) {
            ...
            if (data) {
                let usrID = usr.usrID
                await saveActionState(data.id, "projecttype", "REGISTER", usrID, req, res)
            }
            return res.json(data)
        }
    } catch (error) {
...
self.update = async(req, res) => {
    try {
        let id = req.params.id;
        let body = req.body;
        let data = await projecttype.update(body, {
            where: {
                id: id
            }
        });
...
self.delete = async(req, res) => {
    try {
        let id = req.params.id;
        let data = await projecttype.destroy({
            where: {
                id: id
            }
        });
